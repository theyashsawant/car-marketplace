    ...
    <div className="wrap">
      <div className="page-pad">
        <div className="dash-head">
          <h1>My listings</h1>
          <Link to="/dashboard/car/new" className="btn btn-primary btn-sm">Add a car</Link>
        </div>
          ...
          <div className="rows">
            {cars.map(car => (
              <div className="row" key={car.id}>
                <div>
                  <span className="row-title">{car.year} {car.make} {car.model}</span>
                  <p className="meta">
                    {car.type === 'rental' ? 'Rental' : 'Used car'} — ₹
                    {Number(car.price).toLocaleString('en-IN')}
                    {car.type === 'rental' && ' per day'} in {car.city}
                  </p>
                ...
                <div className="row-right">
                  ...
                  <div className="row-actions">
                    <Link to={`/dashboard/car/${car.id}`}>Edit</Link>
                    <button className="link-btn danger" onClick={() => remove(car.id)}>
                      Delete
                    </button>
                  </div>
                </div>
